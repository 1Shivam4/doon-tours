...
export default function NavbarClient({ links, whatsappNumber, businessName }: Props) {
  ...
  return (
    <header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        dark
          ? 'bg-forest'
          : 'bg-snow border-b border-border shadow-sm'
      }`}
    >
      <div className="max-w-[1280px] mx-auto h-full flex items-center justify-between px-6 md:px-8">
        ...
        <Link
          href="/"
          className={`font-serif text-[22px] font-semibold tracking-[0.01em] transition-colors ${
            dark ? 'text-white' : 'text-bark'
          }`}
        >
          DevBhumi<span className="text-saffron">{businessName.replace('DevBhumi', '')}</span>
        </Link>
